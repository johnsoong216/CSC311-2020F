...
def train_mle_estimator(train_images, train_labels):
    ...
    N = len(train_labels)
    label_count = np.unique(np.argmax(train_labels, axis=1), return_counts=True)[1]
    pi_mle = label_count / N
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    N = len(train_labels)
    label_count = np.bincount(np.argmax(train_labels, axis=1))
    pi_map = label_count / N
    theta_map = (2 + train_images.T @ train_labels) / (4 + label_count)
...
def log_likelihood(images, theta, pi):
    ...
    log_term -= np.log(np.exp(log_term) @ pi).reshape(-1, 1)
    return log_term + np.log(pi)
...
def accuracy(log_like, labels):
    ...
    # YOU NEED TO WRITE THIS PART
    return np.mean(np.argmax(log_like, axis=1) == np.argmax(labels, axis=1))
...
def image_sampler(theta, pi, num_images):
    ...
    # YOU NEED TO WRITE THIS PART
    c = np.random.choice(np.arange(0, 10), size=num_images, p=pi)
    return np.random.binomial(1, theta[:, c]).T
...
def main():
    ...
    avg_loglike_map_t = np.sum(loglike_test_map * test_labels) / len(test_labels)

    print("Training accuracy for MAP is ", train_accuracy_map)
    print("Test accuracy for MAP is ", test_accuracy_map)
    print("Average log-likelihood for MAP Test is ", avg_loglike_map_t)
